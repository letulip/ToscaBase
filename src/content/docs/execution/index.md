...
The **Execution** section of the workspace is where finished TestCases are run and where their results live. This section starts with the basic unit, the ExecutionList, then covers what to do with results, the variants of a run (manual, repeated, recorded, documented, multi-browser), how to take the run out of Commander: on a schedule, from a CI server, or across DEX agents, and finally how self-healing keeps a run alive when the application changes.
...
| [Self-healing mode](/ToscaBase/execution/self-healing/) | Self-healing properties in the Module, the `SelfHealing` parameter (Weighted, Combination), healed steps in the log, Apply self-healing properties |
