...
Coverage specified = относительный вес * процент workstate, агрегированный по привязанным TestCase. Когда все TestCase `Completed`, требование показывает 100%; переведите один в `In Work`, другой в `Planned` — и цифра падает (87% в примере вебинара). Всегда отмечайте готовые TestCase как `Completed`, иначе дашборд занижает покрытие. См. [Структура TestCase и workstate](/ToscaBase/ru/best-practices/test-case-structure/).
