...
:::note
Учебник LambdaGeeks называет пять компонентов — те, что в таблице выше; вводный урок QASCRIPT перечисляет четыре (Commander, Executor, XScan, репозиторий тестов), опуская сервер лицензий, и описывает репозиторий как хранилище «тестовых данных, нужных для выполнения». Оба описывают один и тот же пакет.
:::
