...
| Login | String | `Input` | `{CLICK}` (выберите **Click** из выпадающего списка значений) |

Текстовое поле принимает `Input` со значением; кнопка — не текстовое поле, поэтому её значение — действие «клик», выбранное из списка, при том что ActionMode остаётся `Input`. Все ActionMode описаны в [ActionMode](/ToscaBase/ru/test-cases/action-modes/).
...
Браузер выбран, но ничто его пока не открывает. В `Prerequisites` добавьте TestStep из стандартных модулей, пришедших с шаблоном: откройте раздел Modules (с вкладки **Home**, если вы его закрыли) и найдите **TBox XEngines > HTML > Open Url**, либо нажмите `Ctrl+T` в папке и воспользуйтесь поиском. Перетащите его, переименуйте шаг в `Open Application` и вставьте URL демо-сайта в значение **Url**. Необязательные значения `ActiveTab` и аргументы браузера можно оставить пустыми.
