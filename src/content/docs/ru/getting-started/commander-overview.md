...
Урок по Tosca 16 делит вид по умолчанию на четыре области:

- **Navigation pane (панель навигации)** слева — дерево, дающее обзор вашей работы.
- **Working pane (рабочая панель)** в середине и справа — здесь редактируются отдельные элементы.
- **Заголовки колонок** рабочей панели. Правый щелчок по заголовку — **Column Chooser** — показывает все доступные колонки; двойной щелчок (например, по `Condition`) добавляет её. Чтобы убрать колонку, вытащите её заголовок из строки заголовков, пока не появится крестик, и отпустите.
- **Лента** сверху с вкладками **Home**, **Project**, **View**, **TestCases** и другими, а под ней — вкладки разделов.
...
| **Configurations**, **Test Planning** | Перечислены среди вкладок разделов в уроке по Tosca 16; в этой базе знаний не используются |
| **Tutorial** | Встроенные учебные материалы; для этой базы знаний не нужны |

Урок по Tosca 16 сосредоточен на первых четырёх: Modules — репозиторий объектов (контролы и их локаторы, но богаче, чем репозитории других инструментов), TestCases хранят бизнес-логику, поток и тестовые данные, Execution запускает их и показывает результаты, а Requirements дают трассируемость (процент готовности, состояние выполнения, покрытие), когда TestCase и ExecutionList привязаны к требованиям.

Чтобы увидеть иерархию всего проекта, нажмите **Project** на вкладке **Home**.

### Расположение разделов

Разделы можно показывать рядом. Перетащите вкладку раздела на одну из целей докинга (центр, верх, низ, лево, право), чтобы разделить окно; с **Modules** справа и **TestCases** слева можно перетаскивать Module прямо в TestCase. Правый щелчок по вкладке раздела предлагает **Close**, **Close all but this**, **Float** (раздел становится плавающим окном), **New Vertical Tab Group** и **Move to next tab group**. Закрытый раздел открывается заново через меню **Sections**, где перечислены все разделы; оттуда же раздел можно открыть плавающим окном (показано в уроке по Tosca 16 и в уроке об [отчётах](/ToscaBase/ru/requirements-and-reporting/reports/)).

### Собственные разделы

Разделы по умолчанию — это папки проекта. Чтобы добавить свой, откройте **Project** на вкладке **Home**, щёлкните правой кнопкой корень проекта и выберите **Create Component Folder**; внутри создайте собственные папки Modules, TestCases, Execution и Requirements. Каждая становится отдельным разделом.
...
## Сохранение, отмена и повтор

Кнопки быстрого доступа вверху окна сохраняют workspace, отменяют и повторяют действие. Undo и redo работают только с несохранёнными изменениями: после сохранения история теряется. Сохраняйте осознанно.
