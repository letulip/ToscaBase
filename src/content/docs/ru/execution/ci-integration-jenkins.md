...
Jenkins не может выполнить скрипт `.tcs` напрямую, поэтому нужен bat-файл Windows, вызывающий TCShell со скриптом (в демо — `execute.bat` и `script.tcs`). Как написать оба, описано в [Инструментах командной строки](/ToscaBase/ru/administration/command-line-tools/). Задачи Commander живут в скрипте, поэтому bat-файл никогда не меняется: чтобы сделать что-то другое, меняйте скрипт.
