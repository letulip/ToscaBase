...
| [Управление потоком](/ToscaBase/ru/test-cases/control-flow/) | Операторы `If` с `Then` и `Else`, циклы `While` и `Do`, свойство Maximum repetitions |
| [Повторения](/ToscaBase/ru/test-cases/repetitions/) | Повторение TestStep папки фиксированное число раз через колонку или свойство Repetition, либо число раз, вычисленное из буфера, с `{REPETITION}` |
