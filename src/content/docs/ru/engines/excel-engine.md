...
6. **TBox Close Excel Workbook**: `Workbook name`, `Save` (`True`/`False`), `Path` (другой путь сохраняет копию в другом месте) и `Save as type`. Список типов включает все форматы, которые умеет писать Excel, в том числе книгу, PDF, TXT и CSV, так что TestCase может сразу экспортировать лист в PDF. Тот же Module открывает и эти другие типы.
