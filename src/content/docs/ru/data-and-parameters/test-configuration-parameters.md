...
- Если один и тот же параметр задан на TestCase и на ExecutionList, который его запускает, **значение ExecutionList перекрывает** значение TestCase (урок 11).
...
Урок 11 показывает порядок действий в Tosca 16 для единственного параметра, который нужен каждому веб-TestCase: выделите TestCase в секции **TestCases**, откройте **Test Configuration**, щёлкните запись TestCase правой кнопкой, **Create Test Configuration Parameter**, выберите `Browser`, затем `Chrome` из списка значений, вернитесь в **Details** и запустите в ScratchBook. `{CP[...]}` для него не нужен: системные параметры вроде `Browser` читает сам движок. Типичные применения TCP урок называет так же: браузер, номер релиза, тестовое окружение и идентификаторы подключения или URL, различающиеся по окружениям.

:::note
В уроке 11 среди мест, где можно задать TCP, однажды упомянут «уровень TestStep». У TestStep нет вкладки Test Configuration; уровни — TestCase, папка, ExecutionList и объекты уровня проекта, перечисленные выше.
:::
...
| Mobile | APM server, browser, device model, device name, live view, simulator | Настройки mobile engine, см. [Мобильная автоматизация](/ToscaBase/ru/engines/mobile-automation/) |

:::note
Имена параметров зачитываются со слайда, а не диктуются по буквам; только выделенные кодом подтверждены в написанном виде в других местах этой базы знаний. Точные идентификаторы смотрите в выпадающем списке или документации Tricentis.
:::
