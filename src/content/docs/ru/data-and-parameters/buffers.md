...
:::note
Автор называет то, что вводит на месте суммы, «регулярным выражением», но вслух диктует только `{XB[amount]}`; есть ли перед ним какой-то шаблон, видно лишь на экране. Для этого obstacle достаточно простой подстановки. Регулярные выражения в проверках описаны в [Интервалах и выражениях проверки](/ToscaBase/ru/expressions/intervals-and-verification-expressions/).
:::
