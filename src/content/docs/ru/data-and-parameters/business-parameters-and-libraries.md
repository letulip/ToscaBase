...
1. В многопользовательском workspace сначала возьмите на check-out (взятие на редактирование) родительскую папку ([Многопользовательские workspace](/ToscaBase/ru/administration/multi-user-workspaces/)).
...
Блоки внутри библиотеки перечислены **по алфавиту**, а не в порядке выполнения, так что либо называйте их с учётом этого, либо просто помните, что порядок в библиотеке ничего не говорит о ходе теста.

### Рабочий пример: второй сценарий оформления заказа

В уроке по Tosca 16 сначала собран TestCase для webshop с оплатой кредитной картой, затем нужен тот же поток с оплатой чеком или money order, где отличаются только шаг оплаты и проверка цен (к итогу добавляется комиссия за способ оплаты). Каждый TestStepBlock первого кейса перетаскивается в библиотеку: `Precondition` (открыть URL, войти), `Order Product`, `Start Checkout`, `Checkout`, `Verification of Prices`, `Confirmation`, `Verification of Success`, `Postcondition` (выйти). Новый TestCase собирается перетаскиванием шести из восьми блоков из библиотеки; `Checkout` и `Verification of Prices` тоже перетаскиваются, но затем правятся — см. следующий раздел. Для нового экрана оплаты понадобились новый Module и ещё один TestStep; прогон в ScratchBook затем проверил итог 40 (подытог 25, доставка 10, комиссия 5).

## Правка блока по ссылке: Resolve Reference

Ссылка — не копия. Правка шагов внутри блока по ссылке меняет блок **в библиотеке**, и вместе с ним меняется каждый TestCase, который на него ссылается. В этом смысл при смене свойства контрола (починили блок логина один раз — все TestCase подхватили), и в этом же ловушка, когда одному TestCase нужен вариант.

Для варианта кликните правой кнопкой по блоку-ссылке в TestCase > **Resolve Reference**. Блок становится независимым набором TestStep в этом TestCase (суффикс ссылки и стрелка исчезают), блок в библиотеке и остальные TestCase остаются нетронутыми, и способ оплаты или формулу итога можно менять свободно. Сначала разрешите ссылку, потом правьте.
...
1. Выберите переиспользуемый TestStepBlock в библиотеке и кликните правой кнопкой > **Create Business Parameter Container** (в Tosca 16 пункт предлагается прямо на папке блока).
...
3. В шагах блока **сначала удалите жёстко прописанные значения**, затем **перетащите каждый параметр на TestStepValue**, который должен его использовать. Значение теперь показывает ссылку на параметр в виде `{PL[Username]}` (синтаксис проговаривается в уроке 16: фигурные скобки, `PL`, имя в квадратных скобках). Поле пароля продолжает маскировать значение.
...
Чтобы прогнать тот же поток с другим набором данных, скопируйте TestCase и поменяйте в копии только значения Business Parameters; библиотека не затрагивается. Этим Business Parameters отличаются от [Test Configuration Parameters](/ToscaBase/ru/data-and-parameters/test-configuration-parameters/): оба задаются на уровне TestCase, но Business Parameter питает одну ссылку на один блок, поэтому две ссылки в одном TestCase могут нести разные данные.
