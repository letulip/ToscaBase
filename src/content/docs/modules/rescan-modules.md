...
- A new TestCase needs a control that was not scanned. Rescan and add it to the existing Module rather than scanning again; see [Module hygiene](/ToscaBase/best-practices/module-hygiene/).
...
3. XScan opens with the controls that are already in the Module selected, plus every other control on the page. Refresh if the page is not shown yet.
